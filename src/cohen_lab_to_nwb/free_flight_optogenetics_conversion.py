...
def convert_stimuli_experiment_to_nwb(
    ...
    experiment_iterator = (
        tqdm(range(number_of_experiments), desc="Converting experiments") if verbose else range(number_of_experiments)
    )
    ...
    for experiment_index in experiment_iterator:
        ...
        # Add optogenetic stimulation information
        from pynwb.ogen import OptogeneticStimulusSite, OptogeneticSeries

        device_laser = nwbfile.create_device(
            name="DeviceLaser",
            description="The laser used for optogenetic stimulation",
        )

        ogen_stimuli_site = OptogeneticStimulusSite(
            name="OptogeneticStimulusSite",
            description="Optogenetic stimulation site",
            device=device_laser,
            excitation_lambda=625.0,
            location="TBD",
        )
        ...
        optogenetic_series = OptogeneticSeries(
            ...
            description="Optogenetic stimulation series during the experiment",
            data=data,
            site=ogen_stimuli_site,
            rate=np.nan,
